...
def _worker_alive() -> bool:
    # Heartbeat is the source of truth (updated every ~12s by worker thread)
    if heartbeat_age_sec(WORKER_HEARTBEAT) < 120:
        return True
    lock_pid = _read_lock_pid()
    if lock_pid and pid_is_python(lock_pid):
        # Process exists but heartbeat stale — still count as alive for 3 min grace
...
def main() -> int:
    ...
    # Accepted for supervisor compatibility (older quiet-mode ignored stall kill).
    parser.add_argument("--stall-sec", type=int, default=55)
