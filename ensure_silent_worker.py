...
def _pid_alive(pid: int) -> bool:
    if pid <= 0:
        return False
    if os.name == "nt":
        # Never use tasklist here — even with CREATE_NO_WINDOW it can flash on some hosts.
        try:
...
def _start_worker() -> None:
    # Prefer pythonw via process_guard so no console is allocated.
    try:
        from process_guard import resolve_pythonw, spawn_detached

        py = resolve_pythonw()
        spawn_detached([str(py), str(ROOT / "etrade_worker.py"), "--service"], cwd=ROOT)
        _log(f"Started worker via {py.name} (spawn_detached)")
        return
    except Exception as exc:
        _log(f"spawn_detached failed ({exc}); falling back")
    ...
    if os.name == "nt":
        ...
        try:
            ...
            pass
    argv = [str(py), str(ROOT / "etrade_worker.py"), "--service"]
    if py.name.lower() == "python.exe":
        argv = [str(py), "-u", str(ROOT / "etrade_worker.py"), "--service"]
