...
class SecFilingsAnalyst(BaseExpert):
    ...
    def _collect_filings(self) -> tuple[list[SecFiling], list[str]]:
    ...
    @staticmethod
    def _market_signals(filings: list[SecFiling], by_category: dict[str, int]) -> list[dict[str, Any]]:
        ...
        for category, count in ranked[:3]:
            if count < 1:
                continue
            tickers, bias = category_lookup.get(category, (["SPY"], "NEUTRAL"))
            filing_tickers = sorted({f.ticker for f in filings if f.category == category and f.ticker})
            signals.append(
                {
                    "sector": category.replace("-", " ").title(),
                    "tickers": filing_tickers or tickers,
                    "bias": bias,
                    "reason": f"{count} {category.replace('-', ' ')} filing(s) detected",
                }
            )
            ...
            signals.append(
                {
                    ...
                    "tickers": ["SPY"],
                    "bias": "NEUTRAL",
                    "reason": "No material EDGAR filing clusters detected",
                }
            )
        return signals
    ...
    def analyze(self) -> SecFilingsReport:
        ...
        filings, sources = self._collect_filings()
        ...
        signals = self._market_signals(filings, by_category)
        recs = [
            summary,
