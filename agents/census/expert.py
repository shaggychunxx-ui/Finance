...
class CensusEconomicAnalyst(BaseExpert):
    ...
    def _fetch_retail_sales(self) -> list[SeriesPoint]:
    ...
    def _fetch_housing_starts(self) -> list[SeriesPoint]:
    ...
    def _fetch_business_applications(self) -> list[SeriesPoint]:
    ...
    def _fetch_state_growth(self) -> list[StateGrowth]:
        ...
        if not signals:
            signals.append(
                build_market_signal(
                    ...
                    tickers=["SPY"],
                    bias="NEUTRAL",
                    reason="Census economic indicators show no strong directional tilt",
                    confidence=0.4,
                )
            )
    ...
    def analyze(self) -> CensusReport:
        retail = self._fetch_retail_sales()
        housing = self._fetch_housing_starts()
        bfs = self._fetch_business_applications()
        state_growth = self._fetch_state_growth()
        ...
        return CensusReport(
            ...
            market_signals=self._market_signals(retail_mom, retail_yoy, housing_mom, bfs_mom, state_growth),
