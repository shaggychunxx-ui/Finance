...
class MigrationDataAnalyst:
    ...
    def _fetch_countries(self) -> list[CountryMigrationProfile]:
    ...
    def _market_signals(
        ...
        for c in top_remit[:3]:
            tickers.extend(CORRIDOR_TICKERS.get(c.iso3, []))
        deduped = list(dict.fromkeys(tickers))[:6]
        signals.append({
            "sector": "Remittances & Cross-Border Payments",
            "tickers": deduped or ["WU", "PYPL"],
            "bias": "NEUTRAL",
            "reason": (
                f"Top remittance corridors: {', '.join(c.name for c in top_remit[:3])} "
                f"(${top_remit[0].remittances_usd / 1e9:.1f}B/yr leader)" if top_remit else "No corridor data"
            ),
        })

        bias = "BEARISH" if avg_remit_pct >= 10 else "NEUTRAL"
        signals.append({
            "sector": "Emerging Market Currencies (Remittance-Linked)",
            "tickers": ["EWW", "INDA", "EPHE", "EWZ"],
            "bias": bias,
            "reason": f"Average remittance dependency {avg_remit_pct:.1f}% of GDP across tracked corridors",
        })
        ...
        signals.append({
            ...
            "bias": "NEUTRAL",
            "reason": "Sustained net emigration from tracked corridors sustains elastic US labor supply",
        })
        ...
        if most_dependent is not None:
            signals.append({
                ...
                "bias": "BEARISH" if most_dependent.remittances_pct_gdp >= 15 else "NEUTRAL",
                "reason": (
                    f"{most_dependent.name} remittances equal "
                    f"{most_dependent.remittances_pct_gdp:.1f}% of GDP — high sensitivity to US hiring cycles"
                ),
            })
    ...
    def analyze(self) -> MigrationReport:
        ...
        return MigrationReport(
            ...
            market_signals=self._market_signals(top_remit, most_dependent, avg_remit_pct),
...
def run_migration_analysis(output: Path | None = None) -> dict[str, Any]:
    return MigrationDataAnalyst().run(output=output)
